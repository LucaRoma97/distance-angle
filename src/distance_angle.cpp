...
void odomangleCallback(const std_msgs::Float64& odom)
{
   odomangle = odom.data;
}
...
public:
    ...
    void Computedistangle(const ar_track_alvar_msgs::AlvarMarkers& ar_marker, int i) {
        ...
        xp = ar_marker.markers[i].pose.pose.position.x;
        
        yp = ar_marker.markers[i].pose.pose.position.y;

        x = cos(odomangle)*xp + sin(odomangle)*yp;

	y = -sin(odomangle)*xp + cos(odomangle)*yp;
    ...
    void ComputeRPY(const ar_track_alvar_msgs::AlvarMarkers& ar_marker, int i) {  /* from quaternion to euler angles */
     ...
     void CopytoMarker(distance_angle::DistanceAngle &marker){
	    ...
	    marker.angle = angle;
	    marker.orientation = Yaw + 90 + odomangle; //for having 0 degree into the perpendicular line
    }
...
void distanceangleCallback(const ar_track_alvar_msgs::AlvarMarkers ar_pose_marker)
{
  ...
  for (i=0;i<flags;++i)
	...
	ARmarker.CopytoMarker(marker);
        ...
        ROS_INFO("Distance: [%f], angle: [%f], orientation: [%f]", odomangle, marker.angle, marker.orientation);
...
{ 
 ...
 ros::Subscriber odom_sub = nh.subscribe("/odomangle", 1, odomangleCallback);

  ros::Subscriber sub = n.subscribe("/ar_pose_marker", 1, distanceangleCallback);
